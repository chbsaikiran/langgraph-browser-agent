...
Ported from S9SharedCode/code/browser/driver.py with V9Client replaced by
our LLMClient adapter (Gemini for text, llava:7b for vision).
...
screenshot; the a11y driver sends the legend alone to Gemini.
"""
...
SYSTEM_PROMPT_A11Y = (
    ...
    "goal. You CANNOT see the page — rely on the names.\n"
    "\n"
    "CRITICAL RULES:\n"
    "  - Never bundle `done` with other actions in the same turn.\n"
    "  - Dropdown triggers (ending with ▾ or :) must be the SINGLE action "
    "    that turn — options only appear in the next turn.\n"
    "  - Verify URL / page state on the next turn before declaring done.\n"
    "  - At most 2 actions per turn. Most turns should be ONE action.\n"
    "Be terse in `thinking` — one or two sentences."
)

SYSTEM_PROMPT = SYSTEM_PROMPT_VISION  # back-compat alias
...
class BaseDriver:
    """Shared per-turn loop. Subclasses provide _decide()."""

    SYSTEM_PROMPT: str = ""
    LAYER_NAME: str = "base"

    def __init__(self, page: Page, client: LLMClient, config: DriverConfig):
        self.page = page
        self.client = client
        self.config = config
        ...
        for turn in range(1, self.config.max_steps + 1):
            ...
            if "error" in last.outcome:
                ...
                if failures >= self.config.max_failures:
                    return DriverResult(
                        False, f"gave up after {failures} consecutive failures",
                        steps=self.steps,
                    )
            else:
                failures = 0
...
class SetOfMarksDriver(BaseDriver):
    """Layer 3: screenshot with numbered marks → llava:7b vision call."""
...
class A11yDriver(BaseDriver):
    """Layer 2b: legend-only text call to Gemini. No screenshot, lower cost."""
